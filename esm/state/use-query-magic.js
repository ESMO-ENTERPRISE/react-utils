...
export const useQueryMagic = () => {
    const { mutateQuery, notifyInvalidation, notifyCancellationOfQuery } = useQueryContext();
    /**
     ...
     */
    const setQueryData = useCallback((queryKey, querySetter) => {
        if (!queryKey)
     ...
     */
    const setQueriesData = useCallback((dataQueries) => {
     ...
     */
    const getQueriesData = useCallback((queryKeys) => {
        if (!Array.isArray(queryKeys))
     ...
     */
    const cancelQuery = useCallback(async (queryKey) => {
        if (!queryKey)
            throw new TypeError("Don't provide queryKey as falsy value!");
     ...
     * @param {*} queryKey
     */
    const clearCacheData = useCallback((queryKey) => {
        if (!queryKey)
            throw new TypeError("Don't provide queryKey as falsy value!");
    ...
    const dataQueryMagicInstance = useMemo(() => Object.freeze({
        setQueryData,
        setQueriesData,
        getQueryData,
        getQueriesData,
