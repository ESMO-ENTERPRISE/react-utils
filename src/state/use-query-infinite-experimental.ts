 ...
 * @returns
 */
export const useExperimentalInfiniteQuery = <T = any>(
  ...
  if (!queryKey) {
    throw new TypeError(
      "Provide queryKey as any type except types that don't have toString method especially falsy value"
    );
  }
  ...
  const {
    ...
    isFetchingNextPage,
    isLoading,
  } = useInfiniteQuery<T>(queryKey, fetcher, {
    ...otherOptions,
    onSuccess: () => {
