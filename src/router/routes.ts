import React, {type ReactElement, type ReactNode, Children, isValidElement, useMemo, createElement} from 'react';

import { createMatcher } from './create-matcher';
import { useJsonMemo } from './use-json-memo';
import { type RouteProps, Route } from './route';
...
const Routes = ({ children }: RoutesProps = {}): (React.ReactElement<any, string | React.JSXElementConstructor<any>> | string | number | Iterable<React.ReactNode> | React.ReactPortal | boolean | undefined | null)[] => {
  ...
  return childArray.map((child, i): ReactNode => {
    return isRoute(child)
        ? i === index
            ? [
